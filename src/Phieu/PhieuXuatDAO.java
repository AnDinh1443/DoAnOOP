 ...
 * @author 84907
 */
public class PhieuXuatDAO implements DAOInterface<Phieu>{
    ...
    public int insert(Phieu t) {
		...
		try {
			...
			String sql = "INSERT INTO PhieuXuat (maPhieu, nguoiTao, ngayTao,gioTao,Total) VALUES (?,?,?,?,?)";
    ...
    public ArrayList<Phieu> selectAll() {
		...
		try {
			Connection con = DAO_Phieu.getConnection();
			String sql = "SELECT * FROM PhieuXuat WHERE maPhieu='"+x+"'";
			PreparedStatement pst = con.prepareStatement(sql);
			ResultSet rs = pst.executeQuery();
			while(rs.next()) {
				String maPhieu = rs.getString("maPhieu");
